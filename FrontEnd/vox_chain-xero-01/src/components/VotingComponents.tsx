...
const VotingComponents = () => {
  ...
  const [oriVotingData, setOriVotingData] = useState<votingProps[]>([]);
  ...
  const handleSearch: ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    setVotingData(sliceData?.filter((item) => item.pollName.toLowerCase().includes(target.value)));
  };
      ...
      <div className="flex sm:flex-row flex-col gap-y-5 items-center sm:justify-between mx-auto">
        ...
        <div className="flex-1 flex justify-center gap-6 items-center  px-10">
          ...
          <div className="max-w-[753px] w-full bg-gray-custom rounded-full px-1 py-1">
            <input value={searchVoting} onChange={handleSearch} id={"search-voting"} type="text" className="h-full w-full text-white px-3 outline-none " />
          </div>
        </div>
        {/* create voting */}
          ...
          {votingData.length > 0
            ? votingData.map((value, i) => (
                <div key={i} className="w-full max-w-[18.75rem] h-[11.625rem] flex justify-center items-center  px-4 py-4 rounded-2xl relative bg-purple-dark cursor-pointer">
                  ...
                  {/* Status icon */}
                  <ProgressComponents isCompleted={detailPolls[i].isCompleted} />
                  {/* title */}
